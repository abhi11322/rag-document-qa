"""RAG pipeline: retrieval from Chroma + grounded LLM generation.

Wires together app.vector_store (retrieval) and app.llm_providers
(generation) into a single ask(question) call that returns a grounded
answer plus deduplicated source page references.
"""
...
def retrieve_chunks(question: str, store=None, k: int = DEFAULT_TOP_K):
    """Run similarity search against the persisted Chroma store."""
...
def ask(
    ...
    """Answer a question grounded in the retrieved document context.
    ...
    chunks = retrieve_chunks(question, store=store, k=k)
